 ...
 p2 =   df %>%
  ...
  p3 = df %>%
   ...
   pivot_longer(theo:epdlite, names_to = "K", values_to = "value") %>%
    ...
    mutate(K = factor(K, levels = c("k", "epdlite", "epd", "theo"))) %>%
    ggplot(aes(r, value, color = K, linetype = K)) +
    geom_line(linewidth = 1.75) +
    scale_colour_manual(values = c("#6A3D9A", "#E69F00", "#D55E00", "gray"),
                        labels = c(k = TeX("$\\hat{K}(r)$"),
...
wrap_plots(list(high[[1]], low[[1]], patchy[[1]], patchy_noClust[[1]],
                high[[2]],
                low[[2]] + theme(legend.position = "none"),
                patchy[[2]] + theme(legend.position = "none"),
                patchy_noClust[[2]] + theme(legend.position = "none")),
           ncol = 4)
...
```{r fig_3subjs_kamp, fig.height = 6, fig.width = 12}


wrap_plots(list(high[[1]], low[[1]], patchy[[1]], patchy_noClust[[1]],
                high[[3]],
                low[[3]] + theme(legend.position = "none"),
                patchy[[3]] + theme(legend.position = "none"),
                patchy_noClust[[3]] + theme(legend.position = "none")),
           ncol = 4)
...
ovarian %>% 
  ...
  summarize(n = n(),
            m_cd19 = sum(phenotype == "immune1"),
            m_cd68 = sum(phenotype == "immune2"),
            m_cd19_68 = m_cd19 + m_cd68) %>%
  ungroup()  %>%
  # only 71 subjects have at least 2 B-cells and at least 3 macrophages
  filter(m_cd68 >= 2, m_cd19 >= 2) %>%
  mutate(p_m = m_cd19_68/n,
         p_cd19 = m_cd19/n,
         p_cd68 = m_cd68/n) %>%
  pivot_longer(n:p_cd68, names_to = "stat", values_to = "value") %>%
  group_by(stat) %>%
  summarize(median = median(value),
            q25 = quantile(value, .25),
            q75 = quantile(value, .75),
...
Range of $r$ values where $p$ is significant.
...
es/pp/cindex+ plot_layout(heights = c(1.5, 1.5, 1.5))
```
...
```{r}
# univariate clustering dataset for use in survival analysis
clinical = patient_level_ovarian %>%
  select(sample_id,  age = age_at_diagnosis, stage = stage_bin,
         survival_time, event = death, primary) %>%
  filter(sample_id %in% unique(doc_biv$sample_id)) %>%
...
```{r}
filename = here::here("output",
                      "VPdata_coxPH_results_univariate.RData")

if(file.exists(filename)){
	load(filename)
}else{
  kamp = map_dfr(unique(clinical$r), get_coxph, dat = clinical, 
                 doc_method = "kamp")
  k = map_dfr(unique(clinical$r), get_coxph, dat = clinical, 
              doc_method = "k")
   
  cph = bind_rows(kamp, k)
  
  kamp = map_dfr(unique(clinical$r), get_coxph, dat = clinical, 
                 doc_method = "kamp", get_resi = FALSE)
  k = map_dfr(unique(clinical$r), get_coxph, dat = clinical, 
              doc_method = "k", get_resi = FALSE)

 cph_noResi = bind_rows(kamp, k)
...
```{r supp_fig_vpData_survival_uni, fig.width = 10, fig.height=6}
es/pp/cindex+ plot_layout(heights = c(1.5, 1.5, 1.5))
```
